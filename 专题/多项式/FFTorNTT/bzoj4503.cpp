#include<bits/stdc++.h>
using namespace std;
using ll=long long;
const int maxn=4e5+5;
const int N=1e5+5;
namespace Poly{ 
    //NTT
    int f[maxn],g[maxn],lim,pre[maxn],n,m,rev[maxn];
    const int mod=998244353,G=3,invG=332748118;
    int mypow(int a,int b){ 
        int ans=1;
        while(b){ 
            if(b&1)ans=(ll)ans*a%mod;
            a=(ll)a*a%mod;
            b>>=1;
        }
        return ans;
    }
    int Add(int x,int y){ 
        x+=y;
        if(x>=mod)x-=mod;
        return x;
    }
    int Sub(int x,int y){ 
        x-=y;
        if(x<0)x+=mod;
        return x;
    }
    int mul(int x,int y){ return (ll)x*y%mod;}
    int inv(int x){ return mypow(x,mod-2);}
    void getlim(int x){ 
        lim=1;
        while(lim<x)lim<<=1;
    }
    void initrev(){ 
         for(int i=0;i<lim;++i){ 
            rev[i]=(rev[i>>1]>>1)|((i&1)*(lim>>1));
        }
    }
    void initpre(){ 
        for(int i=1;i<lim;i<<=1){ //一半长度
            int w = mypow(3, (mod - 1) / (i << 1));//g^((p-1)/(n))
            pre[i] = 1;
            for (int j = 1; j < i; j++) pre[i + j] = mul(pre[i + j - 1], w);
        }
    }
    void ntt(int*a,bool tp){ 
        for (int i = 0; i < lim; i++) if (i < rev[i]) swap(a[i], a[rev[i]]);
        for (int mid = 1, cnt = 0; mid < lim; mid <<= 1, cnt++)
            for (int j = 0, len = mid << 1; j < lim; j += len)
                for (int k = 0; k < mid; k++) {
                    int x = a[j + k], y = mul(pre[mid + k], a[j + k + mid]);
                    a[j + k] = Add(x, y);
                    a[j + k + mid] = Sub(x, y);
                }
        if (tp) return;
        int invlim = inv(lim);
        for (int i = 0; i < lim; i++) a[i] = mul(a[i], invlim);
        reverse(a + 1, a + lim);
    }
    void getmul(int n,int m,int*a,int*b){ 
        ntt(a,1);ntt(b,1);
    }
    void clear(){ 
        for(int i=0;i<lim;++i)f[i]=g[i]=0;
    }
}
using namespace Poly;
char S[N],T[N];
int a[maxn],b[maxn],ans[maxn],pos[N];
int main(){
    cin>>S>>T;
    n=strlen(S),m=strlen(T);
    reverse(T,T+m);
    n--;m--;
    for(int i=0;i<=n;++i)a[i]=((S[i]!='?')?S[i]-'a'+1:0),f[i]=a[i]*a[i];
    for(int i=0;i<=m;++i)b[i]=((T[i]!='?')?T[i]-'a'+1:0),g[i]=b[i];
    getlim(n+m+2);
    initpre();
    initrev();
    int sum=0;
    for(int i=0;i<=m;++i)sum+=b[i]*b[i]*b[i];
    getmul(n,m,f,g);
    for(int i=0;i<lim;++i)ans[i]=Add(ans[i],mul(f[i],g[i]));
    clear();
    for(int i=0;i<=n;++i)f[i]=a[i];
    for(int i=0;i<=m;++i)g[i]=b[i]*b[i];
    getmul(n,m,f,g);
    for(int i=0;i<lim;++i)ans[i]=Sub(ans[i],mul(2,mul(f[i],g[i])));
    ntt(ans,0);
    int cnt=0;
    //for(int i=0;i<lim;++i)cout<<ans[i]<<"\n";
    for(int i=m;i<=n;++i){ 
        if(!((ans[i]+sum)%mod))
            pos[++cnt]=i-m;
    }
    cout<<cnt<<"\n";
    for(int i=1;i<=cnt;++i)cout<<pos[i]<<"\n";
    return 0;
}

