...
using ll=long long;
const int mod=1004535809,G=3;//原根
...
int mypow(int a,int b,int p=mod){ 
    int ans=1;
    while(b){ 
...
namespace Poly{ 
    ...
    poly cycleMul(poly a,poly b,int m){ 
        int lim=(int)a.size()+(int)b.size()-1,base=0;
        while((1<<base)<lim)++base;
        a.resize(1<<base);b.resize(1<<base);
        getRevRoot(base);
        ...
        int inv=mypow(1<<base,mod-2);
        for(int i=0;i<lim;++i)a[i]=mul(a[i],inv);
        for(int i=m;i<lim;++i)a[i%m]=Add(a[i%m],a[i]);
        a.resize(m);
        return a;
    }
