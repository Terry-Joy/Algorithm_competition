...
1.树上找点$u$使得$\sum dis(u,v)*u$最小

2.单点修改点权

思路：
考虑$u$为当前决策点，对于其儿子$v$,移向其儿子的权值改变是$(-sum[v]*edge[i]+(sum[u]-sum[v])*edge[i])$

改变小于0的时候，$sum[u]<2*sum[v]$,显然这样的点只有一个，如果没有修改点权这样我们得到一个$O(n^2)$的算法

但是我们发现局限于深度，再加上带修，所以建出点分树

深度$logn$层，从点分树根出发，对于变小的儿子，直接跳到相应的分治重心跳过去，考虑修改与查询，每个点维护自己对自己的答案，自己对父亲的答案，查询的时候就是在点分树上，加上自己对自己的，减去自己对父亲的，加上父亲对自己的，但同时我们也得加上那条边的贡献，所以还得维护多一个点数的贡献，修改也很简单

注意这个算法的复杂度是$O(20*nlogn)$,取决于每个点的出度
