...
考虑枚举任意两个点，u对v有1的贡献当且仅当v第一个删除，设距离为d，概率是$\frac1{(d+1)}$，转化一下就是树上所有长度的路径数 $\frac {cnt[i]}{i+1}$，但是统计每一种路径的条数我们无法用桶来统计，$O(n^2)$，事实上这是卷积的形式，所以容斥统计用FFT维护即可，为了方便，边数+1即可，注意FFT是卷积过程 ，而这题自己对自己也有贡献，所有0也要算进去。所以已经计算了两条路径正反的情况了，这里留个点分+FFT求所有路径准确条数的板子
